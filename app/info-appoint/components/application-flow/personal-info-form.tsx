  ...
  useEffect(() => {
    ...
    const loadRecipients = async () => {
      try {
        ...
        console.error('수신자정보 로딩 실패:', error);
      } finally {
        setLoading(false);
      }
    };
    ...
    if (validateForm()) {
      ...
      selectedResults.forEach(result => {
        if (result === '현재 소속회사') {
          // 선택한 소속회사 정보 사용
          if (formData.company) {
            // 사용자가 직접 입력한 회사 주소가 있으면 사용, 없으면 구성요소 파일에서 사용
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-md p-4">
                  <p className="text-sm font-semibold text-gray-700 mb-3">발송 예정 기관:</p>
                  <div className="space-y-2">
                    {selectedResults.map((result, index) => {
                      // "현재 소속회사"가 선택된 소속회사로 대체
                      if (result === '현재 소속회사') {
                        if (formData.company) {
                          // 사용자가 직접 입력한 회사 주소가 있으면 사용, 없으면 구성요소 파일에서 사용
                          let recipientString = formData.company;
                        ...
                        return (
                          <div key={index} className="flex items-center gap-2 text-sm">
                            <span className="w-2 h-2 bg-blue-600 rounded-full"></span>
                            <span className="text-gray-800">본인 소속회사 (아래에서 소속회사 정보를 입력하세요)</span>
                          </div>
                        );
                      }
          ...
          <div>
            ...
            <input
              ...
              placeholder="본인 소속회사 입력(미입력시 공란으로 남기됩니다)"
            />
            {errors.company && (
          ...
          <div>
            ...
            <input
              ...
              placeholder="본인 소속회사 주소 입력(미입력시 공란으로 남기됩니다)"
            />
          </div>
